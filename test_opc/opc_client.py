...
class UaClient(object):
    ...
    def load_security_settings(self, uri):
        ...
        mysettings = self.settings.get("security_settings", None)
    ...
    def save_security_settings(self, uri):
        ...
        self.settings["security_settings"] = mysettings

    def load_application_certificate_settings(self):
        self.application_certificate_path = None
        self.application_private_key_path = None

        mysettings = self.settings.get("application_certificate_settings", None)
        if mysettings is None:
            return
    ...
    def save_application_certificate_settings(self):
        ...
        self.settings["application_certificate_settings"] = mysettings

    def get_node(self, nodeid):
        return self.client.get_node(nodeid)

    def connect(self, uri, timeout=4000):
        self.disconnect()
        ...
        self.client = Client(uri, timeout)
        self.client.application_uri = uri #self.application_uri
    ...
    def subscribe_datachange(self, node, handler, tperiod=500):
        if not self._datachange_sub:
            self._datachange_sub = self.client.create_subscription(tperiod, handler)
    ...
    def subscribe_events(self, node, handler, tperiod=500):
        if not self._event_sub:
            print("subscirbing with handler: ", handler, dir(handler))
            self._event_sub = self.client.create_subscription(tperiod, handler)
